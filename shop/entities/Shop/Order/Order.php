...
class Order extends ActiveRecord
{
    ...
    public static function guest(CustomerData $customerData, array $items, $cost, $note): self
    {
        $order = new static();
        $order->customerData = $customerData;
        $order->items = $items;
    ...
    {
        if ($this->isPaid()) {
            throw new \DomainException('Заказ оплачен.');
        }
    ...
    {
        if ($this->isSent()) {
            throw new \DomainException('Заказ отправлен.');
        }
    ...
    {
        if ($this->isCompleted()) {
            throw new \DomainException('Заказ завершен.');
        }
    ...
    public function afterFind(): void
    {
        ...
        $this->deliveryData = new DeliveryData(
            ...
            $this->getAttribute('delivery_area'),
            $this->getAttribute('delivery_city'),
            $this->getAttribute('delivery_warehouse')
        );
    ...
    public function beforeSave($insert): bool
    {
        ...
        $this->setAttribute('delivery_area', $this->deliveryData->area);
        $this->setAttribute('delivery_city', $this->deliveryData->city);
        $this->setAttribute('delivery_warehouse', $this->deliveryData->warehouse);
    ...
    public function attributeLabels(): array
    {
        return [
            ...
            'deliveryData.area' => 'Область',
            'deliveryData.city' => 'Город',
            'deliveryData.warehouse' => 'Отделение НП',
            'cost' => 'Стоимость',
